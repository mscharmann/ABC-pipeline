...
ALSO: outputs coalescent times from ms -L option: computes the mean, median, 5-percentile, 95-percentile of the distribution over all loci!
...
#			print "read chrom", slinecnt
			...
			if line.startswith("//"):
				...
				coalescent_times.append( float( line.split("\t")[1]) )
...
# 						print locus_cnt, len(loci_counts_of_replicate)
						...
						sys.stdout.write( "\n" )
						coalescent_time_95perc = np.percentile(coalescent_times, [50, 5, 95])
						f.write( str(sum(coalescent_times)/len(coalescent_times)) + "\t" + str(coalescent_time_95perc[0]) + "\t" + str(coalescent_time_95perc[1]) + "\t" + str(coalescent_time_95perc[2]) + "\n" )
						locus_cnt = 0
						coalescent_times = []
