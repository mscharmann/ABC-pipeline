...
# Python 2.7.6
# vcf_to_genepop.aribtrary_npop.py
# 28 feb 2018
# Mathias Scharmann
		...
		for line in infile:
			if len(line) >= 2:
				...
				if fields[1] in popdict.keys():
					...
					pop_order.append( fields[1] )

	return popdict, pop_order
...
def make_genepop (genotypes_dict, loci_list, popdict, outfilename, pop_order_in_outfile):
	...
	.
	"""
	headerline = "## genepop converted from .vcf for conversion to ms Hudson formats" + "	n_loci:	" + str( len(loci_list) ) + "	pops:	" + str(len( popdict.keys() )) + "	samples:	" + str( sum( [ len(x) for x in popdict.values() ] ) ) 
...
popdict, pop_order = read_pop_info(args.popfile)
pop_order_in_outfile = pop_order
